...
def unicycle_model(current_pos, omega, velocity, dt):
...
def unicycle_controller(current_pos, goal_pos, time_to_complete,dt,steering_pid,velocity_pid):
...
current_pos = (0, 0, 0, 0)  # (x, y, theta, v)
path = [(1, 1, 0, 0)]  # Global path with nodes as a list of tuples
time_to_complete = 5.0  # Desired time to complete the global path in seconds
time_passed = 0.0
...
for goal_pos in path:
    #in order: integral error, previous error
    steering_pid = [0,0]
    velocity_pid = [0,0]
    node_length = math.sqrt((goal_pos[0] - current_pos[0])**2 + (goal_pos[1] - current_pos[1])**2)
    while True:
        remaining_length = math.sqrt((goal_pos[0] - current_pos[0])**2 + (goal_pos[1] - current_pos[1])**2)
        time_to_complete_remaining = time_to_complete*remaining_length/node_length
        dt = 0.1
        
        time_passed += dt
        steering, velocity, steering_pid,velocity_pid = unicycle_controller(current_pos, goal_pos, time_to_complete_remaining,dt,steering_pid,velocity_pid)
        # Update the current position based on the control outputs and the unicycle model dynamics
        current_pos = unicycle_model(current_pos, steering, velocity,dt)
